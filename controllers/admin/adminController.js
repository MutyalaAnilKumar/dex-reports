...
const getCompanyWallets = async (req, res, next) => {
  try {
    ...
    const companyWallets = await companysWallets
...
const createPlatform = async (req, res) => {
  try {
    ...
    if (!platform || typeof platform !== "string") {
      return res
        .status(400)
        .json({ message: "Platform name is required and must be a string." });
    }
    if (!networkId) {
      return res.status(400).json({ message: "networkId is required." });
    }
    ...
    if (platformExists) {
      return res
        .status(409)
        .json({
          message: "Platform with this name already exists in this network.",
        });
    }
  ...
  try {
    ...
    if (platform && platform !== existing.platform) {
      ...
      if (duplicate)
        return res
          .status(409)
          .json({ message: "Platform name already exists in this network." });
    ...
    res
      .status(200)
      .json({ message: "Platform updated successfully.", data: existing });
  } catch (err) {
    res
      .status(500)
      .json({ message: "Error updating platform.", error: err.message });
  }
};
...
const createToken = async (req, res) => {
  try {
    ...
    if (!name || typeof name !== "string") {
      return res
        .status(400)
        .json({ message: "Token name is required and must be a string." });
    }
    ...
    // 🧠 Find platform and populate its networkId (correct field name)
    const platformDoc = await Platform.findById(platformId).populate(
      "networkId",
      "_id name",
    );
    ...
    if (!networkDoc) {
      return res
        .status(400)
        .json({ message: "Platform does not have a linked network." });
    }
        ...
        if (raydiumResponse.ok) {
          ...
          if (
            ...
            if (
              matchingPool &&
              matchingPool.lpMint &&
              matchingPool.lpMint.address
            ) {
              lpMintAddress = matchingPool.lpMint.address;
              console.log(
                `Found lpMint address: ${lpMintAddress} for tokenAddress: ${tokenAddress}`,
              );
            }
          }
        } else {
          console.warn(
            `Failed to fetch pool info from Raydium API for token: ${tokenAddress}`,
          );
        }
      } catch (error) {
...
const getTokens = async (req, res) => {
  try {
    ...
    if (platformId && networkId) {
      message =
        "Tokens for the given platform and network fetched successfully.";
    } else if (platformId) {
...
const addWallet = async (req, res) => {
  try {
    ...
    if (!walletAddress || !tokenId) {
      return res
        .status(400)
        .json({ message: "Both walletAddress and tokenId are required." });
    }
    ...
    const companyWallet = await Settings.findOne({status : true})
    ...
    const newWallet = await Wallet.create({
      walletAddress,
      innerWalletAddress:companyWallet.ourWallet,
      tokenId,
      pairAddress,
      poolType,
      symbol,
...
exports.getWalletOverview = async (
  walletAddress,
  providedTokenAddress = null,
) => {
  if (!walletAddress) {
  ...
  // 🔥 QUICK CACHED RETURN IF TOKEN IS PROVIDED
  if (tokenAddress) {
    const cacheKey = `${walletAddress}:${tokenAddress}`;
    const now = Date.now();
    const cached = walletOverviewCache.get(cacheKey);
    if (cached && now - cached.timestamp < CACHE_TTL) {
      return cached.data;
    }
  }

  // 🔹 Resolve tokenAddress if not provided
  if (!tokenAddress) {
    const tokenDoc = await WalletReport.findOne({ walletAddress });
    tokenAddress = tokenDoc?.token;

    if (!tokenAddress) {
      const walletDoc = await Wallet.findOne({ walletAddress }).populate(
        "tokenId",
        "tokenAddress",
      );
  ...
  // If token not found → safe fallback
  if (!tokenAddress) {
    return {
      total_value: 0,
      solValue: 0,
  ...
  // 🔥 TOKEN-AWARE CACHE KEY (Double check if we didn't check it above)
  const cacheKey = `${walletAddress}:${tokenAddress}`;
  const now = Date.now();
  ...
  try {
    ...
    const overview = {
      ...
      valueInTokens: matchedToken?.balance ?? 0,
      valueInUSD: matchedToken?.value ?? 0,
    };

    walletOverviewCache.set(cacheKey, {
      data: overview,
      timestamp: now,
    });
...
exports.getWalletLastTransaction = async (walletAddress) => {
  ...
  try {
    ...
    if (data?.data?.length > 0) {
      ...
      const walletTokens =
        formattedTransaction.routers.amount1 /
        10 ** formattedTransaction.routers.token1_decimals;
      const solTokens =
        formattedTransaction.routers.amount2 /
        10 ** formattedTransaction.routers.token2_decimals;
...
const getWallets = async (req, res) => {
  try {
    ...
    const tokenAddress = (rawTokenAddress || tokenaddress || "").trim();
    const shouldFilterValue = haveValue === true || haveValue === "true";
    ...
    // Resolve tokenAddress to tokenId if provided
    let resolvedTokenId = null;
    if (tokenAddress) {
      const tokenDocs = await Token.find({
        $or: [
          { tokenAddress: { $regex: new RegExp(`^${tokenAddress}$`, "i") } },
          { solAddress: { $regex: new RegExp(`^${tokenAddress}$`, "i") } },
          { pairAddress: { $regex: new RegExp(`^${tokenAddress}$`, "i") } },
          { lpMint: { $regex: new RegExp(`^${tokenAddress}$`, "i") } },
        ],
      });
      if (tokenDocs.length > 0) {
        const tokenIds = tokenDocs.map((doc) => doc._id);
        filter.tokenId = { $in: tokenIds };
        resolvedTokenId = tokenIds[0];
      } else {
        return res.status(200).json({
          message: "No wallets found for the given token address.",
          count: 0,
          totalPages: 0,
          currentPage: pageNumber,
          limit: limitNumber,
    ...
    const effectiveTokenId = tokenId || resolvedTokenId;

    // Fetch ALL wallets first
    let wallets = await Wallet.find(filter)
      .populate("tokenId", "name tokenAddress")
      .sort({ createdAt: -1 })
      .lean();
    ...
    const concurrencyLimit = pLimit(40);
    await Promise.all(
      wallets.map((wallet) =>
        concurrencyLimit(async () => {
          try {
            const tokenAddress = wallet.tokenId?.tokenAddress;
            const walletOverview = await exports.getWalletOverview(
            ...
            console.error(
              `Error fetching overview for wallet ${wallet.walletAddress}:`,
              err.message,
            );
            wallet.overview = {
              total_value: 0,
    ...
    // Apply haveValue filter if requested
    if (shouldFilterValue) {
      wallets = wallets.filter((w) => (w.overview?.total_value || 0) > 0);
    }
    ...
    // Enrich ONLY paginated wallets with last transaction
    await Promise.all(
      paginatedWallets.map(async (wallet) => {
        try {
          const walletTransaction = await exports.getWalletLastTransaction(
            wallet.walletAddress,
          );
          wallet.transaction = walletTransaction;
        } catch (err) {
          console.error(
            `Error fetching transaction for wallet ${wallet.walletAddress}:`,
            err.message,
          );
    ...
    for (const wallet of wallets) {
      ...
      tokenSummaries[tokenKey].totalWallets += 1;
    ...
    let message;
    if (effectiveTokenId && walletId)
      message =
        "Wallet filtered by both tokenId and walletId fetched successfully.";
    else if (effectiveTokenId)
      message = `Wallets for token: ${
        wallets[0]?.tokenId?.name || "Token"
    ...
    else message = "All wallets fetched successfully.";
...
const getTokenReports = async (req, res) => {
  try {    
    ...
    const pipeline = [
      ...
      },
      {
        $group: {
          ...
          companysLiquidity: { $sum: "$companysLiquidity" },
          usersLiquidity: { $sum: "$usersLiquidity" },

          poolRevenue: { $sum: "$poolRevenue" },
          companysRevenue: { $sum: "$companysRevenue" },
          usersRevenue: { $sum: "$usersRevenue" },

          compoundRevenue: { $sum: "$compoundRevenue" },
          compoundLiquidity: { $sum: "$compoundLiquidity" },
    ...
    const data = await rwaPoolsReport.aggregate(pipeline);

    if (!data.length) {
      return res.status(404).json({
        message: "No datewise data found",
        data: [],
      });
    }

    // ✅ ONLY FIX: apply SAME filter for date count
    const totalDates = await rwaPoolsReport.aggregate([
      { $match: matchFilter },     // ← this line fixes pagination
      { $group: { _id: "$startTime" } },
      { $count: "count" },
    ]);
    ...
    console.error("Datewise aggregation error:", err);
    return res.status(500).json({
      message: "Internal server error",
      error: err.message,
    });
...
//     const filter = { poolType: "cpmm" };

//     if (tokenAddress) filter.tokenAddress = tokenAddress;
//     if (startTime) filter.from_time = startTime;

//     // Count total reports
//     const count = await rwaPoolsReport.countDocuments(filter,poolType="cpmm");
//     const totalPages = Math.max(1, Math.ceil(count / limitNumber));

//     // Fetch paginated reports
//     const reports = await rwaPoolsReport.find(filter,poolType="cpmm")
//       .sort({ createdAt: -1 })
//       .skip(skip)
//       .limit(limitNumber)
//       .lean();
...
const getCompoundWallets = async (req, res, next) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Failed to fetch compound wallets",
        error: error.message,
      });
  }
};
...
const createOrUpdateSettings = async (req, res) => {
  try {
    ...
    await Settings.updateMany({ status: true }, { $set: { status: false } });
...
const CreateAndUpdatePoolWalletData = async (req, res) => {
  try {
    const { updateId, poolId, poolWalletAddress, innerWalletAddress } =
      req.body;
