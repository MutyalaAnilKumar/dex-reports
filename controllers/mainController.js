...
//     const pool=await LiquidityPool.findOne({pairAddress:pairAddress})
...
//       const innerWalletAddressSetting = await SettingsSchema.findOne({ status: "true" });
//     const innerWalletAddress = innerWalletAddressSetting.ourWallet;
...
//     // const resetBuyVolumeInSollllll = resetBuys.reduce((s, t) => s + (t.routers.amount1|| 0), 0);
//     const resetBuyVolumeInSol = resetBuys.reduce((s, t) => s + (t.routers.amount1 / (10 ** t.routers.token1_decimals) || 0), 0);
//     const resetBuyVolume = resetBuyVolumeInSol * addressAverage;
//     // const resetSellVolumeInSolllllll = resetSells.reduce((s, t) => s + (t.routers.amount1|| 0), 0);
//     const resetSellVolumeInSol = resetSells.reduce((s, t) => s + (t.routers.amount1 / (10 ** t.routers.token1_decimals) || 0), 0);
//     const resetSellVolume = resetSellVolumeInSol * tokenAverage;
//     // const agentBuys = buys - resetBuys.length;
...
//     let walletEndBalanceInUSDT = walletEndBalances?.data?.total_value
//     const walletStartBalanceInUSDT = (lastReportWalletBalance && lastReportWalletBalance.walletEndBalanceInUSDT ? lastReportWalletBalance.walletEndBalanceInUSDT : 0) + innerTrasaction;
...
//       rayCost,
//       walletStartBalance,
//       walletStartBalanceInUSDT,
//       walletLoss,
//       walletEndBalance,
//       walletEndBalanceInUSDT,
//       ExpectedCost,
//       priceImpact,
//       slipageAndloss,
//       cost,
...
exports.getAllActivitiesDEFI = async (
  ...
  try {
    ...
    while (hasMoreData) {
      // const url = `https://pro-api.solscan.io/v2.0/account/transfer?address=${walletAddress}&token=${tokenAddress}&fro…
      const url = `https://pro-api.solscan.io/v2.0/account/defi/activities?address=${walletAddress}&token=${tokenAddress}&from_time=${from_time}&to_time=${to_time}&page=${page}&page_size=${pageSize}`;
      const response = await fetch(url, requestOptions);
      const data = await response.json();
    ...
    const priceImpact = settings && solAverage > 0 && pooledSolAverage > 0
      ...
      : 0; //this is price Impact
    const yeild = ((totalVolume * liquidityPool.lpPercentage) * settings.lpRewardPool);
    const ExpectedCost = rayFee + gasFeeInDollars ;//no need
    const cost = rayFee + gasFeeInDollars; //need to add Tip
    ...
    let walletEndBalanceInUSDT = walletEndBalances?.data?.total_value
    const walletStartBalanceInUSDT = (lastReportWalletBalance && lastReportWalletBalance.walletEndBalanceInUSDT ? lastReportWalletBalance.walletEndBalanceInUSDT : 0) + innerTrasaction * solAverage;
    ...
    const totalCost = walletLoss;
    const netCost = totalCost - lpAdd; 
    const slipageAndloss =  totalCost-ExpectedCost; //walletloss-expectedcost
    ...
    const newReport = new WalletReport({
      ...
      walletStartBalance,
      walletStartBalanceInUSDT,
      addedAmount:innerTrasaction,
      addedAmountInUSDT:innerTrasaction * solAverage,
      walletLoss,
      walletEndBalance,
      walletEndBalanceInUSDT,
      ExpectedCost,
...
//(added on 29-01-2026)   commented on 03022026
// exports.getAllActivitiesDEFI = async (
//   tokenAddress,
//   solAddress,
//   chainId,
...
console.log("totalVolumeotherTxs", totalVolumeotherTxs);
    ...
    const buysVolumeInMintAbuyTxs = buyTxs.reduce(
      (sum, tx) => sum + Number(tx.routers.amount1 / (10 ** tx.routers.token1_decimals) ?? 0),
      0
    );
    console.log("buysVolumeInMintAbuyTxs", buysVolumeInMintAbuyTxs);
    const sellsVolumeMintBsellTxs = sellTxs.reduce(
      (sum, tx) => sum + Number(tx.routers.amount1 / (10 ** tx.routers.token1_decimals) ?? 0),
      0
    );
    console.log("sellsVolumeMintBsellTxs", sellsVolumeMintBsellTxs);
    const buysVolumeswapsTxs = buysVolumeInMintAbuyTxs * addressAverage;
    const sellsVolumeswapsTxs = sellsVolumeMintBsellTxs * tokenAverage;
    const totalVolumeSwapTxs = buysVolumeswapsTxs + sellsVolumeswapsTxs;
    ...
    const swapAGGTxs = ACTIVITY_AGG_TOKEN_SWAP.flatMap((tx) =>
      (tx.routers?.swapAGGTxs || []).filter((cr) =>
       ( cr.token1 === address || cr.token1 === tokenAddress) && ( cr.token2 === address || cr.token2 === tokenAddress)
    
      )
    );
    console.log("swapAGGTxs", swapAGGTxs.length);

     // Filter Buys and Sells
     const buyTxsAGG = swapAGGTxs.filter((tx) => tx.token2 === tokenAddress && tx.token1 === address);
     const sellTxsAGG = swapAGGTxs.filter((tx) => tx.token1 === tokenAddress && tx.token2 === address);
...
const totalVolume = totalVolumeSwapTxs + totalVolumeAGG + totalVolumeotherTxs;
console.log("totalVolume", totalVolume);
    ...
    let reportData = {
      ...
      buysVolumeInMintAbuyTxs,
      buysVolumeswapsTxs,
      sellTxs:sellTxs.length,
      sellsVolumeMintBotherTxs,
      sellsVolumeswapsTxs,
      totalVolumeSwapTxs,
      swapAGGTxs:swapAGGTxs.length,
      buyTxsAGG:buyTxsAGG.length,
      buysVolumeInMintAAGG,
      buysVolumeAGG,
      sellTxsAGG:sellTxsAGG.length,
      sellsVolumeMintBAGG,
      sellsVolumeAGG,
      totalVolumeAGG,
      ...
      sellsVolumeotherTxs,
      totalVolumeotherTxs,
      buys:buyTxs.length+buyTxsAGG.length+buyTxsotherTxs.length,
      sells:sellTxs.length+sellTxsAGG.length+sellTxsotherTxs.length,
      buysVolume:buysVolumeAGG + buysVolumeswapsTxs+buysVolumeotherTxs,
      sellsVolume:sellsVolumeAGG+sellsVolumeswapsTxs+sellsVolumeotherTxs,
      totalVolume,
      solAverage,
      addressAverage,
      tokenAverage,
...
exports.getDailyRWAPoolsReports = async (req, res) => {
  try {
    ...
    // Build filter for RWA pool reports
    const filter = {};
    ...
    // Fetch LP percentage (LiquidityPool)
    const pairAddresses = [
      ...new Set(reports.map(r => r.pairAddress).filter(Boolean)),
    ];
    ...
    const pairToLpPercentage = {};
    liquidityPools.forEach(lp => {
      pairToLpPercentage[lp.pairAddress] = lp.lpPercentage ?? 0;
    });

    // Fetch(MM reports)
    // Match by pairAddress + startTime
    const pairStartKeys = reports.map(r => ({
      pairAddress: r.pairAddress,
      startTime: r.startTime,
    }));

    const MMwalletReports = await PoolReport.find({
      $or: pairStartKeys.map(k => ({
    ...
    // Build lookup map for MMobject
    const walletReportMap = {};
    MMwalletReports.forEach(wr => {
      const key = `${wr.pairAddress}_${wr.startTime}`;
      walletReportMap[key] = wr;
    });

    const reportsWithExtras = reports.map(report => {
      const lpPercentage = pairToLpPercentage[report.pairAddress] ?? 0;
      const key = `${report.pairAddress}_${report.startTime}`;

      return {
        ...report,
        lpPercentage,
        poolFee: (report.totalVolume ?? 0) * lpPercentage,
        MMobject: walletReportMap[key] || null, // 👈 attached here
      };
    });
...
//added on 26-12-25  (commented on 02022026)
// const calculateWalletReportForPair = async (pairAddress, startTime) => {
//   try {
//     console.log(`🔎 Finding wallet reports for pairAddress: ${pairAddress}, startTime: ${startTime}`);
...
// added on 02-02-2026  (commenetd on 03022026)
// const calculateWalletReportForPair = async (pairAddress, startTime) => {
//   try {
//     console.log(`🔎 Finding wallet reports for pairAddress: ${pairAddress}, startTime: ${startTime}`);
...
//     // solAverage: take from walletReports with the same pairAddress (generated by getTokenData)
//     let solAverage = 0;
//     if (walletReports && walletReports.length > 0) {
//       const relatedWalletReport = walletReports.find(
//         (report) => report.pairAddress === pairAddress
//       );
//       if (relatedWalletReport) {
//         solAverage = Number(relatedWalletReport.solAverage) || 0;
//       }
//     }
...
//added on 02-02-2026  (commented on 03022026)
// const calculateDailyPoolReportAggregates = async (startTime) => {
//   try {
//     console.log(`📅 Calculating daily pool type aggregates for date: ${startTime}`);
